...
====================================
Runs the income→satisfaction decomposition twice:
  - Unadjusted:  income = coninc (raw family income, constant dollars)
  - HH-adjusted: income = coninc / sqrt(hompop)  (OECD equivalised income)

For each variant, decomposes changes in "Pretty well satisfied" (1972–2024):
  1. Shifting expectations  — how the income→satisfaction curve changed
  2. Rising incomes         — how income growth alone would have affected satisfaction

Weight: wtssps (post-stratification weight).

Data source: data/GSS.xlsx
Outputs:    output/financial/
"""
...
df["satisfied"] = (df["satfin_clean"] == "Pretty well satisfied").astype(float)
...
def run_analysis(df_in, income_col, label):
    ...
    results = pd.DataFrame({
        "year":                              fitted_years,
        "actual_satisfaction":               [year_stats[yr]["actual"] for yr in fitted_years],
        "frozen_income_satisfaction":        [frozen_income[yr]        for yr in fitted_years],
        "frozen_expectations_satisfaction":  [frozen_expect[yr]        for yr in fitted_years],
...
def plot_curves(df_c, income_col, models, title_suffix, filename):
    ...
    ax.set_ylabel("P(Pretty well satisfied) (%)", fontsize=12)
    ax.set_title(f"Income–satisfaction curve by year\n{title_suffix}",
                 fontsize=13, fontweight="bold")
    ax.yaxis.set_major_formatter(mticker.PercentFormatter(decimals=0))
...
def plot_comparison(res_unadj, res_adj, filename):
    ...
    for ax, res, title in zip(
        ...
        yr = res["year"]
        ax.plot(yr, res["actual_satisfaction"] * 100,
                color=COLORS["actual"], lw=2.2, marker="o", ms=3,
                label="Actual satisfaction")
        ax.plot(yr, res["frozen_income_satisfaction"] * 100,
                color=COLORS["frozen_income"], lw=2.2, ls="--", marker="s", ms=3,
                label="Frozen 1972 incomes")
        ax.plot(yr, res["frozen_expectations_satisfaction"] * 100,
    ...
    axes[0].set_ylabel("Share 'Pretty well satisfied' (%)", fontsize=11)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=3,
               fontsize=9.5, framealpha=0.9, bbox_to_anchor=(0.5, -0.04))
    fig.suptitle("Financial satisfaction decomposition: does household-size adjustment matter?",
