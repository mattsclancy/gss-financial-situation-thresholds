"""
GSS General Happiness Analysis — Worst Outcome
================================================
Identical decomposition to gss_happiness.py, but predicting the probability
of the WORST response: "Not too happy" (rather than "Very happy").

Runs both unadjusted (coninc) and HH-adjusted (coninc / sqrt(hompop)) variants.

Weight: wtssps. Data source: data/GSS.xlsx. Outputs: output/happiness_worst/
"""
...
REF_YEAR     = 1972
OUT_DIR      = "output/happiness_worst"
...
df["not_happy"]   = (df["happy_clean"] == "Not too happy").astype(float)
...
def run_analysis(df_in, income_col, label):
    ...
    results = pd.DataFrame({
        "year":                          fitted_years,
        "actual_unhappiness":            [year_stats[yr]["actual"] for yr in fitted_years],
        "frozen_income_unhappiness":     [frozen_income[yr]        for yr in fitted_years],
        "frozen_expectations_unhappiness":[frozen_expect[yr]       for yr in fitted_years],
...
def plot_main_trends(results, title_suffix, filename):
    ...
    yr = results["year"]
    ax.plot(yr, results["actual_unhappiness"] * 100,
            color=COLORS["actual"], lw=2.2, marker="o", ms=4,
            label="Actual 'not too happy' rate")
    ax.plot(yr, results["frozen_income_unhappiness"] * 100,
            color=COLORS["frozen_income"], lw=2.2, ls="--", marker="s", ms=4,
            label="Counterfactual: 1972 incomes, shifting expectations\n"
                  "(rising → unhappiness grew at any given income)")
    ax.plot(yr, results["frozen_expectations_unhappiness"] * 100,
            color=COLORS["frozen_expect"], lw=2.2, ls="-.", marker="^", ms=4,
            label="Counterfactual: current incomes, 1972 expectations\n"
                  "(falling → income growth would have reduced unhappiness)")
    ax.set_xlabel("Year", fontsize=12)
    ax.set_ylabel("Share 'Not too happy' (%)", fontsize=12)
    ax.set_title(f"Unhappiness in the US (1972–2024)\n{title_suffix}",
...
def plot_curves(df_c, income_col, models, title_suffix, filename):
    ...
    ax.set_ylabel("P(Not too happy) (%)", fontsize=12)
    ax.set_title(f"Income–unhappiness curve by year\n{title_suffix}",
                 fontsize=13, fontweight="bold")
    ax.yaxis.set_major_formatter(mticker.PercentFormatter(decimals=0))
...
def plot_comparison(res_unadj, res_adj, filename):
    ...
    for ax, res, title in zip(
        ...
        yr = res["year"]
        ax.plot(yr, res["actual_unhappiness"] * 100,
                color=COLORS["actual"], lw=2.2, marker="o", ms=3,
                label="Actual unhappiness")
        ax.plot(yr, res["frozen_income_unhappiness"] * 100,
                color=COLORS["frozen_income"], lw=2.2, ls="--", marker="s", ms=3,
                label="Frozen 1972 incomes")
        ax.plot(yr, res["frozen_expectations_unhappiness"] * 100,
    ...
    axes[0].set_ylabel("Share 'Not too happy' (%)", fontsize=11)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=3,
               fontsize=9.5, framealpha=0.9, bbox_to_anchor=(0.5, -0.04))
    fig.suptitle("Unhappiness decomposition: does household-size adjustment matter?",
